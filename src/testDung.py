...
def save_img_checkout(img, ma_nhan_vien):
    # path_img = str(datetime.datetime.now()) + '.jpg'
    path_img = f"{ma_nhan_vien}.jpg"
    path_img_checkout = "checkout/" + str(datetime.date.today())
...
def has_existed1(database, ma_nhan_vien):
    query = f"SELECT * from checkout WHERE date(checkout.datetime) == date('now','localtime') and checkout.ma_nhan_vien == '{ma_nhan_vien}'"
    a = database.execute(query)
    database.commit()
    if list(a) == []:
        return 1
...
def insert_checkout(img, ma_nhan_vien, ten_nhan_vien, accuracy):
    global queus
    # queus = [""]
    # path_img = save_img_checkin(img,ma_nhan_vien)
    path = "database//check_in.sql"
    database = sqlite3.connect(path)
    query = f"INSERT INTO checkout (ma_nhan_vien,accuracy,datetime,image_path) VALUES('{ma_nhan_vien}','{accuracy}',datetime('now', 'localtime'),'{path_img}')"

    if has_existed1(database, ma_nhan_vien):
        database.close()
    else:

        inf = " Tạm biệt " + str(ten_nhan_vien) + " "
        queus.append(inf)
        # if len(queus)>1:
        #     queus.drop(0)
        query = f"select * from checkout"
        path_img = save_img_checkout(img, remove_accent(ten_nhan_vien))
        query = f"UPDATE checkout SET datetime=datetime('now', 'localtime')"
        database.execute(query)
        database.commit()
        database.close()
...
def main():
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # cap = VideoStream(src=(
            #     r"rtsp://admin:sp@ce123@192.168.1.49:554")).start()
                    ...
                    if faces_found > 0:
                        ...
                        for i in range(faces_found):
                            ...
                            if best_class_probabilities > 0.75:
                                ...
                                insert_checkout(frame[bb[i][1]-50:bb[i]
                                                      [3]+50, bb[i][0]-50:bb[i][2]+50, :], ma_nhan_vien, infor[0], best_class_probabilities[0])
